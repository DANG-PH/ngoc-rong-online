...
public class HUDFormPopupNhanVatPhai {

    public static void render(SpriteBatch batch,VeHUD veHUD, DuLieuNguoiChoi duLieuNguoiChoi, NhanVat nhanVat,Texture avtNPC) {
        ...
        for (int i = 0; i < soTrangBi; i++) {
            ...
            if (item != null) {
                int kc1 = 0;
                veHUD.layout.setText(veHUD.fontMotaSkill, item.getTenItem());
                veHUD.fontMotaSkill.draw(batch, veHUD.layout, 3 + 70 + 12 + 1020 - 350, y + 49 - 10);
                kc1 += veHUD.layout.width + 5;
                if (item.getSoCap() > 0) {
                    veHUD.layout.setText(veHUD.fontMotaSkill, "[+" + item.getSoCap() + "]");
                    veHUD.fontMotaSkill.draw(batch, veHUD.layout, 3 + 70 + 12 + kc1 + 1020 - 350, y + 49 - 10);
                }
            }
            if (i == 7 && item != null) {
                veHUD.layout.setText(veHUD.fontCapSKill, item.getMoTa());
                veHUD.fontCapSKill.draw(batch, veHUD.layout, 3 + 70 + 12 + 1020 - 350, y + 49 - 30);
            }
            if (i == 6 && item != null) {
                veHUD.layout.setText(veHUD.fontCapSKill, "Hiệu lực trong " + (item.getHanSuDung() > 60f ? (int) (item.getHanSuDung() / 60f) + " phút" : (int)(item.getHanSuDung())+" giây"));
                veHUD.fontCapSKill.draw(batch, veHUD.layout, 3 + 70 + 12 + 1020 - 350, y + 49 - 30);
            }
            if (i == 5 && item != null) {
                int kc = 0;
                ...
                for (int j = 6; j <= 12; j++) {
                    if (item.getChiso()[j] > 0) {
                        ...
                        veHUD.layout.setText(veHUD.fontCapSKill, prefix + chisoduoccong[j] + "+" + item.getChiso()[j] + "%");
            ...
            if (i < danhSachItem.size()) {
                ...
                if (item != null) {
                    ...
                    if (item.getSoLuong() > 1) {
                        veHUD.layout.setText(veHUD.fontsm, item.getSoLuong()+"");
                        veHUD.fontsm.draw(batch, veHUD.layout, 3 + (70- veHUD.layout.width)-5f + 1020 - 350, y + 15f);
                    }
