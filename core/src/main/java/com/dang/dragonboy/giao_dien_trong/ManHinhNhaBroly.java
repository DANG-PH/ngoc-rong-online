...
public class ManHinhNhaBroly implements Screen {
    ...
    public ManHinhNhaBroly(Main game , String tenNhanVat, String hanhtinh ,String nhanvat) {
        this.game = game;
        ...
        cayco = new Texture("map/"+hanhtinh+ "/chung/cayco/" + "cayco1_" + hanhtinh + ".png");
        cayco1 = new Texture("map/"+hanhtinh+ "/chung/cayco/"  + "cayco2_" + hanhtinh + ".png");
    ...
    public void render(float delta) {
        ...
        for (int i = 0; i < 6; i++){
            float skyY = 425 + camOffsetY * 1.0f;
            batch.draw(bautroixd,i*405,skyY,405,150);
            batch.draw(matrang, 750, 490 + camOffsetY * 1.0f, 64, 64);
        }
        // Layer 1: Sky + nuixa
        for (int i = 0; i < 6; i++) {
            float skyY = 280 + camOffsetY * 1.0f;
            batch.draw(sky, i * 1420, skyY, 1420, 150);
            batch.draw(nuixa, i * 510-15, skyY-12, 510, 150);
        }
        batch.end();
        float skyY = 280 + camOffsetY * 1.0f; // Y của sky + nuixa
        shapeRenderer.setColor(180 / 255f, 154 / 255f, 70 / 255f, 1f);
        shapeRenderer.setProjectionMatrix(camManager.camera.combined);
        shapeRenderer.begin(ShapeRenderer.ShapeType.Filled);
        shapeRenderer.rect(0, skyY - 100, 1420, 100); // nằm ngay dưới ảnh
        shapeRenderer.end();
        batch.begin();
        // Layer 2: Nui
        for (int i = 0; i < 4; i++) {
            float nuiY = 190 + camOffsetY * 0.75f;
            batch.draw(nui, i * 510, nuiY, 510, 210);
        }
        batch.end();
        float nuiY = 190 + camOffsetY * 0.75f; // Y của ảnh núi
        shapeRenderer.setColor(115 / 255f, 71 / 255f, 52 / 255f, 1f);
        shapeRenderer.setProjectionMatrix(camManager.camera.combined);
        shapeRenderer.begin(ShapeRenderer.ShapeType.Filled);
        ...
        for (int i = 0; i < 5; i++) {
            float nuithapY = 55 + camOffsetY * 0.5f;
        ...
        tenNpc(font,"Ông Paragus",400,260,40,30);
        ...
        if (targetX > 610 && targetX < 1060) {
            ...
            batch.begin();
            layout.setText(fontDauThan,"Làng Kakarot");
        ...
        if (targetX > 760 && targetX < 990 && targetY >= 0 && targetY <= 400) {
            if (Gdx.input.isKeyJustPressed(Input.Keys.ENTER)) {
                ThongTinChuyenMap info = new ThongTinChuyenMap(nhanVat, "nhabroly",hudRenderer);
