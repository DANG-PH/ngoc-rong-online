...
public class admin_dungle extends renderUInpc {
    ...
    public void renderChucNangNhanQuaThanhCong(SpriteBatch batch) {
        ...
        veHUD.shapeRenderer.end();
        batch.begin();
        veHUD.fontTenSkill.draw(batch,veHUD.layout,(QuanLyCamera.VIRTUAL_WIDTH - 600) / 2f+25,120+ veHUD.layout.height+35);
    ...
    // "Đổi" (nutDuocChonKhiChat == 0) thay vì chờ chạm nút + hết timer.
    public void xuLyEnterGiftCode() {
        nutDuocChonKhiChat = 0;
        xuLyXacNhanGiftCode();
    }

    private void xuLyXacNhanGiftCode() {
